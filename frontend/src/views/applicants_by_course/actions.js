...
import { upsertError } from "../../api/actions/errors";
import { actionFactory, validatedApiDispatcher } from "../../api/actions/utils";
...
export const applicantsByPositionSelector = createSelector(
    [applicationsSelector, (_, positionId) => positionId],
    (applications, selectedPositionId) => {
        const matchedApplications = applications.filter(
            ({ position }) => selectedPositionId === position.id
        );

        const applicantsData = {
            available: matchedApplications,
            selected: []
        };
